...
def generate_search_queries(user_query: str, max_queries: int = 3) -> list[str]:
    ...
    context.append({"role": "system", "content": prompt_load("search_queries.txt").format(max_queries=max_queries)})
...
def generate_answer(user_query: str, content: list[str]) -> str:
    ...
    context.append({"role": "system", "content": prompt_load("answer_synth.txt").format(user_query=user_query)})
